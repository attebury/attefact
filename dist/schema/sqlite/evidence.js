 ...
 */
export const evidence = sqliteTable("attefact_evidence", {
    id: text("id")
        .primaryKey()
    ...
    supersededBy: text("superseded_by").references(() => evidence.id),
    createdAt: integer("created_at", { mode: "timestamp" }).notNull().default(sql `(unixepoch())`),
}, (t) => [
    check("evidence_binding_shape", sql `(${t.pinRef} is not null) <> (${t.contentHash} is not null)`),
    check("evidence_kind_valid", sql `${t.kind} in ('repo', 'commit', 'merged_pr', 'deploy', 'talk_external', 'certification', 'package_release', 'document', 'screen_recording')`),
